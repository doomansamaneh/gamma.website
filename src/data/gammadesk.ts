export interface GammaDeskFeature {
  name: string;
  slug: string;
  description: string;
  group: string;
...
export interface GammaDeskCapabilityGroup {
  name: string;
  slug: string;
  description: string;
  features: GammaDeskFeature[];
}

export const gammaDeskCapabilityGroups: GammaDeskCapabilityGroup[] = [
  {
    name: 'مدیریت خدمات',
  ...
  },
  {
    name: 'یکپارچه‌سازی',
    slug: 'integration',
    description: 'نقاط اتصال GammaDesk با سامانه‌های دیگر.',
    features: [
...
export const gammaDeskFeatures = gammaDeskCapabilityGroups.flatMap((group) => group.features);